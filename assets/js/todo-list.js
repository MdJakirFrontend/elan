...
function addItem(listArry) {
    ...
    listArry.forEach(function (value, key) {
        // store data in str
        str += "<li class='todo-item' data-key=" + key + " >" + value + "<button onclick='removeItem(this.parentNode)' class='btn btn-danger ti-close float-right rounded-0' style='margin-left:3px' ></button> <button onclick='editItem(this.parentNode)' class='btn btn-warning ti-marker-alt float-right rounded-0'></button> </li>";
    });
    // append to markup
    list.innerHTML = str;
    // item count
